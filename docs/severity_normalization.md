...
AIID does not ship a numeric severity score. We derive severity from `title` and `description` using keyword heuristics in `qualitative_harm_to_severity()`.
...
MITRE ATLAS is a taxonomy source, not an incident feed. ATLAS techniques do not carry severity scores.

---

## Defaults

When no signal is available, severity defaults to `informational`. This keeps unmapped or sparse records in the dataset without falsely inflating risk scores.
